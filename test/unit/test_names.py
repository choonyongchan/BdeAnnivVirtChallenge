...
resolution normalises both sides, then falls back to word-order-insensitive and
fuzzy matching fitted globally by fit() -- which must stay strictly one-to-one
and must refuse to guess when a username was declared by several people.
...
def test_contested_name_is_not_rescued_by_the_fuzzy_tier(tmp_path, monkeypatch):
    """A blocked name must stay blocked in every tier, not fall through to fuzzy."""
    roll = _fitted(tmp_path, monkeypatch, [
        ["HOO JUN HAO", "40SAR", "Cougar", "NSF", "Jun Hao"],
        ["CHIN JUN HAO", "41SAR", "Hawk", "NSF", "jun hao"],
        ["ALICE ANON", "40SAR", "Cougar", "NSF", "Alice Anon"],
